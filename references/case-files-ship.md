...
**A logged run, 2026-09-01.** A ~2,000-line step in a CLI tool (a new package, a new
command, a new config section) went through `/code-review` at high effort. It returned
10 findings, every one real and reproduced. They were fixed carefully — excluded
...
- The fix for "an existing output file gets clobbered" added an overwrite switch by
  reusing an earlier step's flag, whose documented meaning is "ignore the recorded judgement". A
  user re-running with that flag for that meaning would now silently lose an edited file.
- The fix for "excluded zones are dropped" suppressed *every* structural overlap
