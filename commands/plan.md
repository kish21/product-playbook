...
> **Open `PRINCIPLES.md` and `MECHANISMS.md` before Step 0** (paths above; the other two only when a rule points into them), then apply them — load-bearing here: **per-feature contract (testable "done")**,
