...
4. **Give it a real parent reference, then publish it.** Resolve the parent ticket's *issue number* from the
   dedup index already fetched in Step 2 and write `#N` into the ticket file, so GitHub renders the
   bidirectional timeline link. **A parent with no published issue degrades to the plain ID with the reason
   stated — never a guessed number**, which would link the bug to an unrelated issue. Only then commit the file
   and publish it as a single issue with the classification as a label, its ticket file as the body, unchanged
   (`publishing.md` §An issue body is its ticket file), subject to the same dedup and remote guards from Step 2.
   It carries its lane's `lane:` and `owner:` labels and gets its card on the Delivery Board with all four
   fields set (`publishing.md` §The Delivery Board).
