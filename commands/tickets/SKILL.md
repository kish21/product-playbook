...
> **This skill's OWN reference files are a DIFFERENT folder** — they live in `references/` **beside this SKILL.md** (`commands/tickets/references/`), not in the plugin-root `references/` named above. Both install routes put them there. Below, `adhoc-capture.md` · `publishing.md` · `slicing.md` · `tickets-md.md` · `verification.md` each mean that folder: open them by that path, and never look for them next to MECHANISMS.md.
...
- **State model** (`STATE-MODEL.md` §2c): writes no spine section · `declined` ✓ · `override` ✓ · `superseded` n/a — writes `TICKETS.md` + `docs/issues/*`, not a spine section; a re-run skips tickets that already exist, so there is nothing in the spine to erase
- **Exit criteria:**
