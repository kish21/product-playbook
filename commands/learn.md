...
description: >
  ...
  for recurring metric checks. Loops back to /scope or /plan for the next cycle.
---
...
> **Open `PRINCIPLES.md` and `MECHANISMS.md` before Step 0** (paths above; the other two only when a rule points into them), then apply them — load-bearing: **honesty**, **measure-first**, **vision-alignment re-check**,
