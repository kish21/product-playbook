...
> **Open `PRINCIPLES.md` and `MECHANISMS.md` before Step 0** (paths above; the other two only when a rule points into them), then apply them — load-bearing: **typed contracts not raw dict/text**, **units/scale/shape
