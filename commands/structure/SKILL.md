...
- **A shape-changing re-run after `/tickets` rewrites every ticket's path list — named AND planned files — and closes the finding; lanes, order and owners stay.** Stale paths send the next `/build` to recreate the folder just emptied; published issues keep the old paths until `/tickets` re-runs and syncs them (`publishing.md` §Dedup index). (case file: The move that came a phase late)
