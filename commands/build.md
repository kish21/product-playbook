...
4. **Run + verify the LIVE path** — compose `/run` and `/verify` to exercise the path the product actually runs, then **trace your change to its real callers** (green unit tests ≠ wired in).
   ...
   - **Giving an existing data path a SECOND READER? Audit the OLD code's assumptions, not the new code.** A field that was safe to leave stale, unvalidated or unrefreshed *because nothing read it* becomes a defect the moment something does — and the new code reviews clean, because the bug is an omission upstream that used to be free. (case file: The field that was safe until someone read it)
   - **"The generated command/graph/config CONTAINS X" is not "the engine ACCEPTED X" — run it.** And assert the output against the PRODUCER's own computed value, never against a number you re-derived by reading the code: re-deriving is a second guess, and it can be wrong in the same direction twice. (case file: The duration that reading the code got wrong twice)
