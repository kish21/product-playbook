...
> War stories: `${CLAUDE_PLUGIN_ROOT}/references/case-files-build.md` (plugin root, beside MECHANISMS.md — not this skill's `references/`).
...
- **Writes:** a row in `PRODUCT.md#Build log` + `docs/features/<feature>.md` — or, in its place, the one per-ticket record `#Project policy` names.
...
- **Read `#Project policy` first, when the spine has one** (`MECHANISMS-ON-DEMAND.md` §Project policy): a project that wraps `/build` in its own command declares its rules there once — the per-ticket record, never merge, no deploy — and they apply here without the wrapper restating them.
...
5. **Review the diff — one round, then the user decides.**
   ...
   - **Round 1.** Compose `/code-review` and, on any auth/data surface, `/security-review` — in parallel, **both BEFORE the push, the PR and the close**, so their verdicts land in the record. Fix every finding inside the files this ticket changed, each with its proof, and **commit the fixes locally** before any round 2 — for the same reason.
...
6. **Document** — once, after Step 5's last round — write/update `docs/features/<feature>.md`; reconcile it with the code. **`#Project policy` names a per-ticket record** → write that content THERE, once, and nothing into `docs/features/`; the `#Build log` row stays, the one-line index `/ship` and `/dev-check` read. **Copy every `evidence:` number from the command's captured output, never ahead of it** — a figure pencilled in while CI runs reads exactly like a measured one. (case file: The pencilled bundle size)

## Step 3 — Write back to `PRODUCT.md`
Append ONE table row to `#Build log` — `| feature | DoD-incl-security met? | how verified: command → result, plus Step 5's round scope | docs/features/<feature>.md |`. **A row is one line, and `#Build log` holds table rows and nothing else** — besides the dated one-line `Override` / `_Not run_` records §Declined runs puts at its top; no paragraph under the table, which the next build copies. Findings, defects, round detail and **Step 3c's result go in the feature doc**, never in the log (`MECHANISMS-ON-DEMAND.md` §Section is a record). **Find your place with `grep -n` — the section heading, your ticket's id — and never load the log**: a log every build reads makes each ticket cost more than the last. (case file: The log every build read)
...
- (UI features) built-to-the-design → **`/frontend-audit`** 0 errors against `DESIGN.md` — or 0 new with `--baseline`, the pre-existing count quoted.
...
Per `MECHANISMS.md` §Step 3c, check what this phase just produced against decisions **already recorded** — here: `#Contracts` (types crossing boundaries), `#Architecture` (adapters — no vendor SDK in logic), `#Scope` (non-goals) and `DESIGN.md` (UI tokens) — the richest surface for contradiction, because this is the phase that writes real code. On a conflict, **name both sides, ask which wins, and update the loser** (fix the artefact, or add a dated `superseded by` line to the earlier section) — never leave it standing in two places. Adding detail to an earlier decision is not a contradiction. **Step 3c's result goes in the feature doc** — never a paragraph in `#Build log`, which holds table rows and nothing else.
