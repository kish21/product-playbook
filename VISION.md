...
| 1 Product | `/vision` | Vision, customer, problem, value; benchmark to the current-year market; metric/JTBD/risk/model | `PRODUCT.md#Vision` has a one-sentence vision + who/problem/value/verified-market-read + north-star metric + job-to-be-done + riskiest assumption + business model, all non-empty |
...
| 2 Dev | `/architect` | Decide stack + tools + key decisions (benchmarked to 2026, chosen on the project's constraints); patterns/anti-patterns; ADRs | `#Architecture` records stack+tools+why **+ the constraint set each choice was optimised against** (incl. lock-in/exit cost), externals behind adapters, the design patterns applied + anti-patterns avoided (current-year), migrations approach |
...
| 2 Dev | `/tickets` | Breaks down milestones into epics of structured GitHub issue tickets with target file paths, modules, tasks, and DoD, planned in TICKETS.md | TICKETS.md, docs/issues/*.md, .github/ISSUE_TEMPLATE/ |
