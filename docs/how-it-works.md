...
- [`../tools/session_cost.py`](../tools/session_cost.py) — what a phase run cost, read from the session log (calls, wall and agent working minutes, tokens, ≈ $), so a close never estimates.
...
| **Dev** | `/tickets` | Splits each milestone into **epics** of independently mergeable tickets — **one per behaviour a user can see**, as many as the milestone has — **grouped into module lanes** (one per `STRUCTURE.md` module, ordered inside, parallel across) with an Owner — **vertical** thin slices or **horizontal** layer tickets, proposed and confirmed by you — and files them on a **Delivery Board** (Status · Owner · Lane · Seat), each epic a parent issue with its tickets as sub-issues, so one person or four can take the same backlog. Writes the plan to **`TICKETS.md`** — epics, lanes, a flow graph of which lane waits on which, and who starts what on day one — never status, which stays on GitHub. Exact file paths, typed in/out, security DoD. Given a description instead, logs ONE ad-hoc bug against the owning file | `TICKETS.md` + `docs/issues/*` + issue/PR templates | After `/contracts`, before building; or any time you spot a bug |
...
| 🛠️ `/build` | `/build M1-PARTY-01` | Reuse before writing · verifies the **live path** · `/code-review` on the diff · commits | Feature code + `docs/features/<name>.md`; ends with the plain close — built, verified how, what you do next | Green tests over a dead live path |
